...
void Esp8266::run() {
  if (SYS_CFG.nextLoopInterval()) {
    ...
    _mqttService.publish("weather-station", _bmp280.getJsonValue());

    // SYS_CFG.deepSleep();
  }
}

void Esp8266::write(const char *name, JsonObject &json) {
  LOG.verbose(F("%s|%s"), name, esp8266util::toString(json).c_str());
  Firebase.set(name, JsonVariant(json));
  if (Firebase.failed()) {
    _logger.error(F("Saving %s values to Firebase failed."), name);
  }
}
