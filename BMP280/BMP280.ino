...
void setup() {
  ...
  if (bmp280.begin(0x76, BMP280_CHIPID)) {
    VERBOSE_FP(F("BMP280 is ready for %s"), bmp280.getDeviceName().c_str());
    // read sensor data
    bmp280.update(USE_MOCK_DATA);
    // serialize sensor data
    StreamString* payload = new StreamString();
    size_t size = bmp280.serialize(*payload);
    VERBOSE(*payload);
    // push sensor data
    WiFiClient client;
    HTTPClient http;
    ...
    ERROR_FP(F("Setup BMP280 failed!"));
  }
